...
if __name__ == "__main__":
	...
	for s in sense:
		...
		for id in env_ids["open"]:
			...
			d[s] += [float(c[i]) / float(t[i]) for i in range(len(c))]
			# d[e] += s
	# for e in envs:
	...
	axs[0,0].set_ylabel("coverage speed (%/s)")
	...
	fig.tight_layout()
	# plt.show()
	plt.savefig("4zitu.pdf", bbox_inches='tight')
	...
	# axs[1].set_ylabel("#clicks / #completion_time")
	...
	# bp1 = axs[1].boxplot(data, showmeans = True, meanprops = mp1, widths=0.5, sym='+', patch_artist=True, flierprops = dict(markerfacecolor='#d4d4d4',markeredgecolor="#666666"))
	# plt.setp(bp1['boxes'], facecolor='#b2b2b2', linewidth=0)
	# axs[1].set_title("(b) Effectiveness")
	# axs[1].set_ylabel("coverage rate")
	# axs[1].set_xticklabels(["Open","Urban","Indoor","Natural"])
	#
	# fig.tight_layout()
	# plt.show()
	# plt.savefig("random_results1.pdf", bbox_inches='tight')
